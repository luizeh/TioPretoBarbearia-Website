                            ...
                            foreach ($weekDays as $day):
                                ...
                                if ($emBloqueio) {
                                    ...
                                    foreach ($inicioAppointments as $appointment):
                                        ...
                                        $horaIni     = substr($appointment['hora_inicio'], 0, 5);
                                        $iniMin      = ((int) substr($horaIni, 0, 2)) * 60 + (int) substr($horaIni, 3, 2);
                                        $horaFim     = sprintf('%02d:%02d', intdiv($iniMin + $durationMinutes, 60), ($iniMin + $durationMinutes) % 60);
                                        ...
                                        echo '<span class="agenda-appt__time"><i class="fa-regular fa-clock"></i>' . $horaIni . '–' . $horaFim . '</span>';
                                        echo '<span class="agenda-appt__service">' . htmlspecialchars($appointment['servico']) . ' · ' . (int) ($appointment['duracao_minutos'] ?? 30) . ' min</span>';
                                        echo '<span class="agenda-appt__status">' . htmlspecialchars($statusLabel) . '</span>';
